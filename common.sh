...
ProcessVPK ()
{
	...
	do
		...
		~/ValveResourceFormat/Decompiler/bin/Release/netcoreapp3.1/linux-x64/publish/Decompiler --input "$file" --output "$(echo "$file" | sed -e 's/\.vpk$/\//g')" --vpk_cache --vpk_decompile --vpk_extensions "vxml_c,vjs_c,vcss_c,vsndevts_c,vpcf_c,json,txt,cfg,res,pop,gameevents,png,jpg,gif"
